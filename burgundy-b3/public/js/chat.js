	...
	function display() {
		$.get('/api/chat/', function (data) {
			...
			if (data.length > 0) {
				$('#chat_id').empty();
				for (var index = 0; index < data.length; index++) {
					if (data[index].userId == parseInt(sessionStorage.userId) && data[index].other_user == parseInt(sessionStorage.user2)) {
						var newDiv = $('<div>');
